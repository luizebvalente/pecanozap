...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Por simplicidade, vamos usar o email como token
        # Em produção, use JWT ou similar
        user = User.query.filter_by(email=token, is_admin=True, is_active=True).first()
        if not user:
...
def admin_login():
    try:
        ...
        user = User.query.filter_by(email=email, is_admin=True, is_active=True).first()
        if not user or not user.check_password(password):
            return jsonify({'error': 'Credenciais inválidas ou usuário não é admin'}), 401
        
        # Por simplicidade, retornar email como token
        # Em produção, use JWT
        return jsonify({
...
def create_initial_data():
    ...
    try:
        ...
        admin_email = 'admin@pecanozap.com'
        if not User.query.filter_by(email=admin_email, is_admin=True).first():
            ...
            admin_user = User(
                ...
                is_active=True,
                city_id=1,  # São Paulo
                category_id=1  # Restaurantes (placeholder)
            )
