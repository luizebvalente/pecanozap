...
def api_root():
    return jsonify({
        'message': 'API Peça no Zap funcionando!',
        'version': '1.0',
        'status': 'online',
        'endpoints': [
            '/api/categories',
...
def get_businesses():
    try:
        ...
        # Query base com joins para incluir city e category
        query = User.query.filter_by(is_active=True).options(
            db.joinedload(User.city),
            db.joinedload(User.category)
        )
...
def get_business(business_id):
    try:
        business = User.query.filter_by(id=business_id, is_active=True).options(
            db.joinedload(User.city),
            db.joinedload(User.category)
        ).first()
        if not business:
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Verificar se é o token de admin válido
        if token != 'admin_token_123':
            return jsonify({'error': 'Token de admin inválido'}), 403
...
# =====================================================
# ROTAS DE MAPA E PROXIMIDADE
# =====================================================

@app.route('/api/businesses/nearby', methods=['GET'])
def get_nearby_businesses():
    """
    Busca estabelecimentos próximos a uma localização específica
    Parâmetros:
    - lat: latitude do usuário
    - lng: longitude do usuário  
    - radius: raio de busca em km (padrão: 5km)
    ...
    try:
        ...
        # Buscar todos os estabelecimentos com coordenadas
        ...
        for business in all_businesses:
            ...
            if distance <= radius:
                ...
                business_data['distance'] = round(distance, 2)
...
@app.route('/api/businesses/<int:business_id>/location', methods=['PUT'])
@admin_required
def update_business_location(business_id):
    """
    Atualiza a localização de um estabelecimento (apenas admin)
    """
    try:
        data = request.get_json()
